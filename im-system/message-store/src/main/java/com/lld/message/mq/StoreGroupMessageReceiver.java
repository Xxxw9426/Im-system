...
public class StoreGroupMessageReceiver {
    ...
    @RabbitListener(
            bindings = @QueueBinding(
                    value=@Queue(value= Constants.RabbitConstants.StoreGroupMessage,durable = "true"),      // @Queue绑定我们的队列，durable:是否持久化
                    exchange = @Exchange(value=Constants.RabbitConstants.StoreGroupMessage,durable = "true")    // @Exchange绑定交换机，durable:是否持久化
    ...
    public void onChatMessage(@Payload Message message , @Headers Map<String,Object> headers,
        ...
        try{
            // 首先将队列中的消息由json字符串形式转化为我们设置的DoStoreGroupMessageDto
            JSONObject jsonObject = JSON.parseObject(msg);
            // 由于jsonObject中没有ImMessageBodyEntity类，所以此时转化是空，我们要对它进行赋值
            DoStoreGroupMessageDto messageDto = jsonObject.toJavaObject(DoStoreGroupMessageDto.class);
            // 获取jsonObject中的ImMessageBody
            ImMessageBodyEntity messageBody = jsonObject.getObject("messageBody", ImMessageBodyEntity.class);
            // 赋值
            messageDto.setImMessageBodyEntity(messageBody);
            // 持久化我们的消息
            storeMessageService.doStoreGroupMessage(messageDto);
            // 返回ack
